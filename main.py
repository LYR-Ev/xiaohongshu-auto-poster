...
class XiaohongshuAutoPoster:
    ...
    def create_and_publish_post(self, word: str = None, theme: str = None, level: str = "CET-4") -> dict:
        ...
        try:
            ...
            print("\n📝 步骤1: 生成AI文案...")
            if theme == "word":
                # 单词来源：未指定 word 时从该 level 单词库中选一个「未在 posts 中出现过」的词
                if word and word.strip():
                    word_for_post = word.strip()
                else:
                    try:
                        has_posted = (lambda w, l, p: self.recorder.has_posted(w, l, p)) if self.recorder else (lambda w, l, p: False)
                        word_for_post = self.content_generator.pick_unused_word(level, has_posted)
                    except AllWordsUsedError as e:
                        print(f"❌ {e}")
                        return {
                            "success": False,
                            "error": str(e),
